...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `getcat` (IN `cid` INT)  SELECT * FROM departments WHERE dept_id=cid$$
...
INSERT INTO `student_info` (`user_id`, `first_name`, `last_name`, `email`, `password`, `mobile`, `address1`, `address2`) VALUES
...
--
DELIMITER $$
CREATE TRIGGER `after_student_info_insert` AFTER INSERT ON `student_info` FOR EACH ROW BEGIN 
INSERT INTO student_info_backup VALUES(new.user_id,new.first_name,new.last_name,new.email,new.password,new.mobile,new.address1,new.address2);
END
$$
DELIMITER ;
...
--
-- Table structure for table `student_info_backup`
--
...
--
-- Dumping data for table `student_info_backup`
--

INSERT INTO `student_info_backup` (`user_id`, `first_name`, `last_name`, `email`, `password`, `mobile`, `address1`, `address2`) VALUES
...
--
-- Indexes for table `student_info_backup`
--
ALTER TABLE `student_info_backup`
  ADD PRIMARY KEY (`user_id`);
...
--
-- AUTO_INCREMENT for table `student_info_backup`
--
ALTER TABLE `student_info_backup`
  MODIFY `user_id` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=26;
...
--
ALTER TABLE `orders_info`
  ADD CONSTRAINT `user_id` FOREIGN KEY (`user_id`) REFERENCES `student_info` (`user_id`);
