...
if(isset($_POST["categoryhome"])){
	$category_query = "SELECT * FROM departments WHERE dept_id!=1";
	...
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$cid = $row["dept_id"];
			$cat_name = $row["dept_title"];
            
            $sql = "SELECT COUNT(*) AS count_items FROM products,departments WHERE product_cat=dept_id";
...
if(isset($_POST["getProducthome"])){
	...
	$product_query = "SELECT * FROM products,departments WHERE product_cat=dept_id LIMIT $start,$limit";
...
if(isset($_POST["gethomeProduct"])){
	...
	$product_query = "SELECT * FROM products,departments WHERE product_cat=dept_id AND product_id BETWEEN 71 AND 74";
	$run_query = mysqli_query($con,$product_query);
	if(mysqli_num_rows($run_query) > 0){
...
if(isset($_POST["get_seleted_Category"]) ||  isset($_POST["search"])){
	if(isset($_POST["get_seleted_Category"])){
		$id = $_POST["dept_id"];
		$sql = "SELECT * FROM products,departments WHERE product_cat = '$id' AND product_cat=dept_id";
	}else {
		$keyword = $_POST["keyword"];
		$sql = "SELECT * FROM products,departments WHERE product_cat=dept_id AND product_keywords LIKE '%$keyword%'";
	}
